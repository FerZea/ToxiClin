...
from expedientes.models import (
    ...
    CatViaIngreso, CatUbicacionEvento,
)
...
class FiltroHistoriaForm(forms.Form):
    ...
    paciente = forms.CharField(
        required=False,
        label='Paciente (nombre o apellido)',
    ...
    sexo = forms.ModelChoiceField(
        ...
        required=False, label='Sexo', empty_label='— Todos —',
    )
    tipo_contacto = forms.ModelChoiceField(
        queryset=CatTipoContacto.objects.filter(activo=True),
        required=False, label='Tipo de contacto', empty_label='— Todos —',
    )
    motivo_consulta = forms.ModelChoiceField(
        queryset=CatMotivoConsulta.objects.filter(activo=True),
        required=False, label='Motivo de consulta', empty_label='— Todos —',
    )
    circunstancia = forms.ModelChoiceField(
        queryset=CatCircunstanciaNivel1.objects.filter(activo=True),
        required=False, label='Circunstancia', empty_label='— Todas —',
    )
    tipo_agente = forms.ModelChoiceField(
        queryset=CatTipoAgente.objects.filter(activo=True),
        required=False, label='Tipo de agente', empty_label='— Todos —',
    )
    severidad = forms.ModelChoiceField(
        queryset=CatSeveridad.objects.filter(activo=True),
        required=False, label='Severidad', empty_label='— Todas —',
    )
    evolucion = forms.ModelChoiceField(
        queryset=CatEvolucion.objects.filter(activo=True),
        required=False, label='Evolución', empty_label='— Todas —',
    )
    via_ingreso = forms.ModelChoiceField(
        queryset=CatViaIngreso.objects.filter(activo=True),
        required=False, label='Vía de ingreso', empty_label='— Todas —',
    )
    # Corrección 3 — RF-17 exige filtro por ubicación del evento
    ubicacion_evento = forms.ModelChoiceField(
        queryset=CatUbicacionEvento.objects.filter(activo=True),
        required=False, label='Ubicación del evento', empty_label='— Todas —',
    )
    ...
    # ── Rango de edad ─────────────────────────────────────────────────────
    # Se ingresa en años; la vista convierte a días/meses/años
    # para cubrir lactantes y niños menores de 1 año (corrección 2).
    edad_min = forms.IntegerField(
        required=False,
        label='Edad mínima (años)',
        min_value=0,
        widget=forms.NumberInput(attrs={'placeholder': 'ej: 0', 'min': 0}),
    )
